...
def _render_skills_index(
    ...
    return (
        "## Skills\n"
        "Before replying, scan the skills below. Load a skill with skill_view(name) and follow its "
        "instructions when it materially improves correctness, safety, or execution of the current task. "
        "Choose the narrowest skill that owns the workflow; broad topic overlap alone is not enough. "
        "Skills can supply specialized commands, proven workflows, and the user's conventions even "
        f"when you already know how to use basic tools like {_basic_tools}. "
        "Load additional skills or dependencies only when their stated condition is met or they supply "
        "a distinct, necessary capability for the current step; do not load every related skill or "
        "reference preemptively. Do not skip applicable safety instructions or required prerequisites.\n"
        ...
        + "\n".join(index_lines) + "\n"
        "</available_skills>\n\n"
        "Proceed without loading a skill when none would materially help the current task."
        + hidden_note
    )
